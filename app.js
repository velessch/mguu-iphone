(function(){
'use strict';
if(window.__mguuWebV029){return;}
window.__mguuWebV029=true;

const APP_VERSION='0.30 Web';
const DEFAULT_GROUP={id:'000000230',name:'24ГМУ-СКР11.1'};
const BASE_URL=new URL('/portal/student/scheduler1.php',window.location.origin).href;
const RATING_URL=new URL('/portal/student/rating.php',window.location.origin).href;
function toPortalProxyUrl(value){
  try{
    let u=new URL(value,window.location.href);
    if(u.hostname==='portal.mguu.ru')return new URL('/portal'+u.pathname+u.search+u.hash,window.location.origin).href;
    return u.href;
...
function nativeNotificationAction(action,payload){
  ...
  if(action==='request'){
    ...
    Notification.requestPermission().then(function(result){
      if(result==='granted'){
        ...
        navigator.serviceWorker&&navigator.serviceWorker.ready.then(function(reg){return reg.showNotification('МГУУ',{body:'Уведомления включены',icon:'/icon-192.png',badge:'/badge-96.png',tag:'mguu-permission'});}).catch(function(){});
  ...
  if(action==='show'){
    ...
    navigator.serviceWorker.ready.then(function(reg){
      return reg.showNotification(payload.title||'МГУУ',{body:payload.body||'',icon:'/icon-192.png',badge:'/badge-96.png',tag:'mguu-'+String(payload.id||Date.now()),data:{notificationId:String(payload.id||'')},renotify:false});
    }).catch(function(){});
    return;
  }
}
...
async function sdoApi(url,options){
  let response=await fetch(url,Object.assign({credentials:'same-origin',cache:'no-store',headers:{'accept':'application/json'}},options||{}));
...
function showError(msg){let c=document.getElementById('content');if(c)c.innerHTML='<div class="empty"><div class="emptyTitle">Расписание не загрузилось</div><div class="muted">'+esc(msg)+'</div><button id="retry" class="primary">Повторить</button></div>';let b=document.getElementById('retry');if(b)b.onclick=()=>loadMain(false);}
