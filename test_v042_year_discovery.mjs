...
console.log('PASS v0.42: future academic years are discovered dynamically, cached, and added without changing selected rating period');
need(/const ratingPeriodRefreshState=\{busy:false,lastAt:0\};/,'academic-year refresh state is not explicitly initialized');
need(/if\(ratingPeriodRefreshState\.busy\)return false;/,'busy guard is not using initialized refresh state');
need(/ratingPeriodRefreshState\.busy=true;ratingPeriodRefreshState\.lastAt=now;/,'refresh state is not set before request');
need(/finally\{ratingPeriodRefreshState\.busy=false;\}/,'refresh state is not released in finally');
if(/\bratingPeriodOptionsBusy\b|\bratingPeriodOptionsLastAt\b/.test(app))throw new Error('stale undeclared loose refresh-state variables remain');
console.log('PASS v0.42: academic-year refresh state dependencies are explicitly initialized and guarded');
